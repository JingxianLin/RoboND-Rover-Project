...
def search_rocks(img, thresh=(110, 110, 50)):
    color_select = np.zeros_like(img[:,:,0])
    check_thresh = (img[:,:,0] > thresh[0]) & (img[:,:,1] > thresh[1]) & (img[:,:,2] < thresh[2])
    color_select[check_thresh] = 1
    return color_select
...
def perception_step(Rover):
    ...
    searched = search_rocks(warped)
    if searched.any():
        rock_x, rock_y = rover_coords(searched)
        ...
        Rover.vision_image[:,:,1] = searched * 255
    else:
